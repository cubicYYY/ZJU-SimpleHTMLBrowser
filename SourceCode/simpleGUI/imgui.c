...
int button(int id, double x, double y, double w, double h, char *label)
{
	...
	// 菱形按钮 
	MovePen(x, y+0.2*h);
	if( gs_button_color.fillflag ) StartFilledRegion(1); 
	{
		DrawLine(0, 0.6*h);
		DrawLine(0.2*h, 0.2*h);
		DrawLine(w-0.4*h, 0);
		DrawLine(0.2*h, -0.2*h);
		DrawLine(0, -0.6*h);
		DrawLine(-0.2*h, -0.2*h);
	...
	if( gs_button_color.fillflag ) EndFilledRegion();
	...
	if( gs_UIState.clickedItem==id && gs_UIState.mousedown ) 
	{	
	if( gs_button_color.fillflag ) StartFilledRegion(1); 
	{
		MovePen(x, y+0.2*h);
 ...
 */
 ...
 /*修改库增加的变量*/ 
 #define MAX 1000
char textbuf2[MAX][MAX]; //传递网址（文件名） 
extern int fact; //判断是否按过回车 
extern int pageFlash,newflag,Tabflag;
extern int tabNumber;
int textbox(int id, double x, double y, double w, double h, char textbuf[], int buflen)
{
	char * frameColor = c1.letter;
	char * labelColor = c1.letter;
	int textChanged = 0;
	...
	if (notInMenu(gs_UIState.mousex, gs_UIState.mousey) &&
		inBox(gs_UIState.mousex, gs_UIState.mousey, x, x + w, y, y + h) ) 
	{
		frameColor = c1.letter;
		labelColor = c1.letter;
		gs_UIState.actingMenu = 0; // menu lose focus
		if ( gs_UIState.mousedown) {
			gs_UIState.clickedItem = id;
		}
	...
	if ( gs_UIState.kbdItem == id )
	{
		...
		{
		case VK_RETURN:
				strcpy(textbuf2[tabNumber],textbuf);	
				strcpy(textbuf,""); 
				pageFlash=1;
